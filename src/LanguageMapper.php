...
/**
 * Maps the Drupal language to the configured language.
 */
class LanguageMapper {

  /**
   * Returns the language code configured in the request translator.
   *
   ...
   */
  public static function getMappedLanguageCode(string $langcode, TranslationRequestInterface $translation_request): string {
   ...
   *   The Drupal language code.
   */
  public static function getDrupalLanguageCode(string $langcode, TranslationRequestInterface $translation_request): string {
    $configuration = $translation_request->getTranslatorProvider()->getProviderConfiguration();
    $mapping = $configuration['language_mapping'] ?? [];
    foreach ($mapping as $drupal => $mapped) {
      if ($mapped == $langcode) {
        return $drupal;
      }
    }

    // In case it's not configured, we just lower the case of the language code.
